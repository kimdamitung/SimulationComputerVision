...
def hexagon(img, goto_X, goto_Y, radius = 210, rotate = 90, n = 6):
    ...
    for i in range(n):
        ...
        points.append((x, y))
        if i in [0, 2, 4]:
            cv2.line(img, (goto_X, goto_Y), (x, y), color_blue, thickness)
        cv2.circle(img, (x, y), 2, color_red, thickness)
        cv2.circle(img, (goto_X, goto_Y), radius, color_blue, thickness=2)
    for i in range(n):
        cv2.circle(img, points[i], int(radius / 3), color_red, thickness)
